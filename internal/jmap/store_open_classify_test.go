...
// could reach. A third would do the same, so the file that opens the store
// must not build one.
func TestTheStoreSeamDoesNotClassifyByHand(t *testing.T) {
	body, err := os.ReadFile("mailboxmethods.go")
	if err != nil {
		t.Fatalf("read the seam: %v", err)
	}
	for i, line := range strings.Split(string(body), "\n") {
		if strings.Contains(line, "ErrServerFail") || strings.Contains(line, "ErrServerUnavailable") {
			t.Errorf("mailboxmethods.go:%d builds its own dependency answer: %s\n"+
				"classify through storeFailure instead -- one outage, one story", i+1, strings.TrimSpace(line))
		}
	}
}
