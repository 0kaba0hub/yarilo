...
func TestSubstituteVars(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "%u in quoted string not substituted",
			driver:    "mysql",
			query:     "SELECT CONCAT('%u literal') AS mail FROM t WHERE username = %u",
			username:  "alice@example.com",
			wantQuery: "SELECT CONCAT('%u literal') AS mail FROM t WHERE username = ?",
			wantArgs:  []any{"alice@example.com"},
		},
		{
			name:      "all vars postgres unquoted",
			driver:    "postgres",
		...
		{
			name:      "sandbox pattern: %u in CONCAT string + WHERE",
			driver:    "mysql",
			query:     "SELECT CONCAT(home, ':', home, ':VOLATILEDIR=/tmp/%u') AS mail FROM t WHERE username = %u",
			username:  "u1@d1.test",
			wantQuery: "SELECT CONCAT(home, ':', home, ':VOLATILEDIR=/tmp/%u') AS mail FROM t WHERE username = ?",
			wantArgs:  []any{"u1@d1.test"},
		},
	}
