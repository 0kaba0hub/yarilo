...
UPDATE mailbox SET password = '$HASH';
INSERT INTO mailbox (username, password, maildir, local_part, domain)
SELECT
  CONCAT('u', n, '@d00001.test'),
  '$HASH',
  CONCAT('u', n, '@d00001.test/'),
  CONCAT('u', n),
  'd00001.test'
...
SELECT COUNT(*) AS total FROM mailbox;
" 2>&1 | grep -v Warning
