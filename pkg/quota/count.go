...
// Unreadable or absent folders are skipped rather than failing the whole read,
// mirroring how the aggregate self-heals: a transient per-folder error must not
// deny service on the user-wide total.
func CountUsage(idx FolderVSizer, folders []string) Usage {
	var u Usage
