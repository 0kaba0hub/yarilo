...
For **mdbox** the reactive rebuild is phased (#594 Phase 2). Corruption is *detected* at the read boundary (a map miss, a vanished `m.<N>`, or a truncated/bad record surfaces as corrupt storage, distinct from transient I/O), and the storage scan quarantines a corrupt `m.<N>` instead of aborting. Because mdbox messages are shared across folders through the map, a per-folder rebuild would import unrelated messages — so the per-folder endpoint (`/api/backend/index/rebuild`) returns `501` for mdbox and there is a dedicated **storage-wide rebuild**: `POST /api/backend/index/rebuild-storage {user}` (or `yarilo-admin backend index rebuild-storage <user>`). It reconciles the shared map against the physical `m.<N>` files under the storage lock, resets every folder index to the messages that still exist, **recomputes each map record's refcount from the actual folder references** (Dovecot `rebuild_apply_map` parity), drops map records whose message vanished, and bumps a persisted `rebuild_count` generation counter in the map header. It **refuses** to run when the scan is incomplete (a half-corrupt `m.<N>` or transient I/O — move/repair the named file and re-run) or when a configured alt tier is unmounted (would mass-expunge alt-resident mail). It is an operator repair tool (Dovecot `force-resync` parity) and should run with **delivery to the user quiesced** — the delivery folder-append is not serialised on the storage lock, so a message delivered concurrently can have its refcount miscomputed.

A message present on disk but referenced by no folder becomes **zero-ref** (reported as `unreferenced_zeroref` and logged) so the next purge reclaims it — it is deliberately **not** re-filed into a mailbox. Without `ORIG_MAILBOX` metadata the rebuild cannot distinguish genuinely-lost mail from churn/refcount-leak garbage, and blind adoption mass-resurrects deleted mail; true orphan restore lands with `ORIG_MAILBOX` in a later phase.

Known parity gaps of the storage-wide rebuild (same as Dovecot / the sdbox heal): `ResetFolder` stamps a fresh modseq on every surviving record (a modseq bump for QRESYNC clients) and loses VANISHED tombstone fidelity for dropped UIDs; FTS may keep ghost documents for expunged messages until the next `fts rescan`. The reactive auto-trigger for mdbox (marking + heal-on-open) and routing orphans back to their original mailbox (ORIG_MAILBOX metadata, instead of the INBOX fallback) are tracked as follow-ups.
