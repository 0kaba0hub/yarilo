...
For **mdbox** the reactive rebuild is phased (#594 Phase 2). Corruption is now *detected* at the read boundary (a map miss, a vanished `m.<N>`, or a truncated/bad record surfaces as corrupt storage, distinct from transient I/O), and the storage scan quarantines a corrupt `m.<N>` instead of aborting. Until the storage-wide rebuild lands (Phase 2b) mdbox has **no** healer, so the FSCKD marker is *not* set for it (the marker is gated on a driver that can clear it) and the operator per-folder rebuild endpoint returns `501` for mdbox — a per-folder rebuild is unsafe against mdbox's storage-wide, folder-agnostic scan.
