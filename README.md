...
Director replicas self-organize into a ring at runtime (#750 phase 1 — replaces the earlier static full-mesh `peers` list, #700): members are ordered by `(ip, port)` and each dials only its right neighbor, never a full mesh. Every member count is a fully valid, service-serving state (never refuses service) — a lone director is an ordinary N=1 ring, no peer machinery runs at all. `components.director.peers` is now a **seed list**: each entry is tried in turn for a one-time `DIRECTOR-JOIN`, after which membership maintains itself via propagation — the headless `<release>-director-ring:9102` Service (#748/#751) is the intended seed, its DNS name resolving directly to every ready pod's IP so any resolver hit lands on a live replica, despite director being a plain Deployment rather than a StatefulSet. `components.director.ring_secret` (auto-generated into Secret `<release>-director-ring-secret`, mirroring the API token) authenticates joins via HMAC-SHA256 — leaving it unset rejects every join attempt outright, so that replica can only ever run standalone. `components.director.min_members` (default 3) is an install-time warning only, no runtime effect. Phase 1 covers ring topology, membership propagation, and the HMAC join core; full user/backend state snapshot on connect (phase 3) and members_hash anti-entropy (phase 4) are tracked separately.

**Phase 2 (#750)** adds `components.director.join_allowed_nets` (CIDRs allowed to even attempt a `DIRECTOR-JOIN`, checked before the HMAC challenge — same fail-fast pattern as the admin API) and dial-back verification (the acceptor dials a joiner's *claimed* address back and confirms a director actually answers there before admitting it, guarding against a valid-secret joiner self-reporting an unreachable or wrong address). It also fixes ring TLS to verify the peer certificate against a stable configured hostname (`components.director.ring_tls_server_name`) instead of the ephemeral pod IP being dialed, which could never match a realistic certificate — **but today's shared `internalTLS` cert has no per-component SAN at all, so enabling `internalTLS` on the director without first reissuing that cert to include this name makes ring TLS fail closed** (the ring silently never converges past N=1). Keep `internalTLS.enabled: false` on the director component until #753 (chart-managed director certificate) lands, or you reissue the shared cert with the SAN yourself; the director process also warns at startup if its loaded certificate is missing it.
