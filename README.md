...
**#765 follow-up (live rolling-restart still accumulated phantom members — timer tuning changed nothing)** added the missing eviction layer. Dial-based death detection only ever covers a node's *current right neighbor*, so a member parked outside everyone's dial path — a terminating pod whose snapshot a fresh joiner merged mid-restart (the ring headless Service publishes not-ready addresses by design) — was never probed by anyone and lingered forever. Now every seed-poll cycle each node re-verifies **every** member it knows with the authenticated JOIN poll (concurrent, short per-probe deadline bounding dial + handshake); a member failing `components.director.poll_evict_failures` consecutive cycles (default 3) is tombstoned and its removal broadcast. Tombstones now also carry a TTL (`components.director.tombstone_ttl`, default 600s) so churn across many rollouts can't grow the set unboundedly — safe because a resurrected-but-unreachable member is re-evicted by the probe layer within a few cycles regardless.
