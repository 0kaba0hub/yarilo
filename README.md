...
**#776 (backend DNS re-resolution + pruning)** brings the same DNS-authoritative liveness the ring uses for its own members to the BACKEND list. The backend pod IPs were resolved once at startup and never re-resolved, so a rescheduled backend's dead IP kept a hash-slice of users blackholing (`dial backend … i/o timeout`) until every director restarted — and stale IPs gossiped ring-wide and never pruned. Each director now re-resolves the `mail_servers` headless DNS every `components.director.backend_refresh_interval` (default 30s) and reconciles its backend list to the live endpoint set: new pod IPs added, IPs no longer resolvable (a reschedule, or a stale entry a peer gossiped) removed ring-wide via `RING-CHANGE down` (an actual `RemoveBackend`, not a health mark). DNS is authoritative for backend **existence**; the monitor sidecar still owns health/drain. `admin`-API-added backends are never pruned, and a failed/empty resolution of a host never touches that host's backends (no total blackhole on a DNS blip).
